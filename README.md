...
This library transforms JSONata stanzas into valid Erlang code that is meant to be executed in the presence of a `Msg` map. For example:
...
Functions are defined in two places [in the parser](src/erlang_red_jsonata_parser.yrl) or in the [evaluator](src/erlang_red_jsonata.erl).
